...
//  L5_s
//
//  Created by Dasha Neyzhmak on 28.04.21.
...
TREE* root; // Первый узел в дереве

int count = 0;

TREE* create_tree(TREE* root); // Создать корень дерева

TREE* add_list(TREE* root);

TREE* add_node(int i); // Добавить узел

TREE* search_node(TREE* root); //Поиск по заданному ключу

void delete_node(int key); // Удалить узел дерева

void print_tree(TREE* t, int level); // Вывести дерево на экран
...
int printGivenLevel(TREE* node, int level);// Определить число листьев на каждом уровне дерева

void clear_memory(TREE* t); // Очистить память
...
delete_node(key); break;

case 5:

print_tree(root, 0); break;
...
bool isLeaf(TREE* node) { //Функция проверки, что узел дерева является листом
...
int printGivenLevel(TREE* node, int level) { //Функция подсчета листьев на заданном уровне
