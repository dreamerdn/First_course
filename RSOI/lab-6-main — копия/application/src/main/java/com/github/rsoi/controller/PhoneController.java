...
public class PhoneController {
    ...
    private int countMatchingParameters(Phone phone, int rammin, int rammax) {
        int matches = 0;
        if (phone.getRammin() >= rammin && phone.getRammax() <= rammax) {
            matches++;
        }
        // add more matching parameters if needed
        return matches;
    }
   ...
   @GetMapping("/getPhoneNames")
    ...
    @PostMapping("/addPhoneImage")
    public ResponseEntity addPhoneImage(@RequestParam("id") Long id, @RequestParam("imagLink") String imagLink) {
        try {
            Phone phone = phoneRepository.findById(id).orElse(null);
            if (phone == null) {
                return ResponseEntity.badRequest().body("Телефон с указанным ID не найден");
            }
            phone.setImageLink(imagLink);
            phoneRepository.save(phone);
            return ResponseEntity.ok("Ссылка на картинку успешно добавлена");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Произошла ошибка");
        }
    }
    ...
    @DeleteMapping("/deletePhoneImage")
    public ResponseEntity deletePhoneImage(@RequestParam("id") Long id) {
        try {
            Phone phone = phoneRepository.findById(id).orElse(null);
            if (phone == null) {
                return ResponseEntity.badRequest().body("Телефон с указанным ID не найден");
            }
            phone.setImageLink(null);
            phoneRepository.save(phone);
            return ResponseEntity.ok("Ссылка на картинку успешно удалена");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Произошла ошибка");
        }
    }
